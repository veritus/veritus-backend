...
class SoupUtilsGetCaseDetailsTestCase3(TestCase):
    """
    Test get_case_details in soupUtils.py
    """
    ...
    def setUp(self):
        ...
        self.details_soup = BeautifulSoup(details_xml, features='xml')

    def test_related_cases(self):
        rel_cases_gen = soupUtils.get_attribute_value(
            self.details_soup, 'mál', 'málsnúmer'
        )
        rel_cases = []
        for rel_case in rel_cases_gen:
            rel_cases.append(rel_case)
    ...
    def test_subj_id(self):
        ...
        subj_id_gen = soupUtils.get_attribute_value(
            self.details_soup, 'efnisflokkur', 'id')
        subject_names = []
    ...
    def test_get_gase_details(self):
        status, rel_cases, subj_ids, case_creator_names = x_p.get_case_details(self.details_soup)
